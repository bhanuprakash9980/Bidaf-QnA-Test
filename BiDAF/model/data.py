...
class SQuAD():
    def __init__(self, args, path):
        ...
        self.CHAR_NESTING = data.Field(
            batch_first=True, tokenize=list, lower=True)
        self.CHAR = data.NestedField(self.CHAR_NESTING, tokenize=word_tokenize)
        self.WORD = data.Field(
            batch_first=True, tokenize=word_tokenize, lower=True, include_lengths=True)
        self.LABEL = data.Field(
            sequential=False, unk_token=None, use_vocab=False)
        ...
        if os.path.exists(dataset_path):
            ...
            self.test = data.TabularDataset(
                path=f'{path}/{args.test_file}l', format='json', fields=self.dict_fields)
            torch.save(self.test.examples, test_examples_path)

            print("loading train & dev dataset...")
            train_examples = torch.load(train_examples_path)
            dev_examples = torch.load(dev_examples_path)
            self.train = data.Dataset(
                examples=train_examples, fields=self.list_fields)
            self.dev = data.Dataset(
                examples=dev_examples, fields=self.list_fields)
        ...
        if args.context_threshold > 0:
            self.train.examples = [e for e in self.train.examples if len(
                e.c_word) <= args.context_threshold]

        print("building vocab...")
        self.CHAR.build_vocab(self.train, self.dev)
        self.WORD.build_vocab(self.train, self.dev,
                              vectors=GloVe(dim=args.word_dim))
        ...
        self.train_iter, self.dev_iter, self.test_iter = \
            data.BucketIterator.splits((self.train, self.dev, self.test),
                                       batch_sizes=[
                                           args.train_batch_size, args.dev_batch_size, args.test_batch_size],
                                       device=self.device,
